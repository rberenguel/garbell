...
Walks the project using `rg --files` (respects `.gitignore`), parses source files with language-specific heuristics, and writes a sharded JSON chunk map to `~/.garbell/indexes/<workspace-hash>/`.
